...
└── src/
    ...
    │   ├── page.tsx           # TOP画面（左: 未完了案件の固定幅一覧 / 右: 検索パネル。完了済みは常に除外）
    │   ├── completed/page.tsx # 完了済み一覧（`TaskTable`で全件表示）
    │   ├── tasks/
...
- TOP画面（`/`）は `getTasks({ onlyIncomplete: true, ... })` により、ステータス絞り込みの有無に関わらず常に完了済みタスクを除外する。完了済みタスクの確認・参照は `/completed`（`getTasks({ status: "completed" })`）に一本化している。
